...
_CHANNEL_CONTEXT: dict[str, str] = {
    ...
    "missive": (
        ...
        "\n"
        "## v32 — server-side send-vs-draft gate\n"
        "STEP 1, STEP 2, STEP 3 and order-confirmation acknowledgements\n"
        "are AUTO-SENT by the server (customer sees them in seconds). STEP\n"
        "4 (the email with the binding price + [QUOTE_READY] + PDF) is\n"
        "the ONLY one drafted for Justin's review. Always write TO THE\n"
        "CUSTOMER (\"Hi <Name>, ...\"). Never address Justin in the body —\n"
        "he's the operator, not the recipient.\n"
        "\n"
        ...
        "\n"
        "  STEP 4 \u2014 All info collected: send the PDF\n"
        "    Trigger: specs complete, customer_has_own_artwork set, AND\n"
        "    funnel fields present (delivery_method + delivery_address if\n"
        "    delivery + is_company + is_returning_customer).\n"
        "    Action sequence on this turn:\n"
        "      1. CALL save_customer_info(...) with everything captured\n"
        "         across the thread.\n"
        "      2. CALL the pricing tool with the right needs_artwork:\n"
        "         \u2022 own artwork or pending-later \u2192 needs_artwork=false\n"
        "         \u2022 design service             \u2192 needs_artwork=true,\n"
        "                                         artwork_hours=1.0\n"
        "      3. Compose the FINAL reply using the price the tool\n"
        "         returned, ending with [QUOTE_READY] so the PDF attaches:\n"
        "          Hi <Name>,\n"
        "          Thanks for those details. For <qty> <product> <specs>,\n"
        "          the total comes to \u20ac<price from tool> including VAT.\n"
        "          I've attached the full branded quote as a PDF.\n"
        "          Turnaround is 3-5 working days from when we have\n"
        "          print-ready artwork. Reply to this email to confirm\n"
        "          the order or with any adjustments.\n"
        "          Best,\n"
        "          Justin\n"
        "          Just Print\n"
        "          [QUOTE_READY]\n"
